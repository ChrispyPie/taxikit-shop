window.TAXIKIT_BUILD = "1.1.0-wip69";
window.TAXIKIT_CHANGELOG = [
  {
    ver: "1.1.0-wip69",
    date: "2026-09-04 · under utveckling",
    items: ["Båt rad 3: Kiel/Göteborg/På väg. Kaj i expansion"]
  }
];
...
(function restyleRows() {
  if (window.__taxikitRowUi69) return;
  window.__taxikitRowUi69 = true;
  ...
  css.textContent =
    ...
    ".feed-item.dir-hide{display:none!important}.feed-map{color:#8cb4ff;text-decoration:underline}";
  ...
  function tidyBoatExtra(row) {
    ...
    if (/stena danmark/i.test(title)) {
      rows.push(kvHtml("Kaj", '<a class="feed-map" target="_blank" rel="noopener" href="https://maps.google.com/?q=57.701195,11.947163">Danmarksterminalen</a>'));
      ...
      rows.push(kvHtml("Kaj", '<a class="feed-map" target="_blank" rel="noopener" href="https://maps.google.com/?q=57.6938,11.9144">Tysklandsterminalen</a>'));
  ...
  function boatDurMin(title) {
    if (/kiel/i.test(title)) return 870;
    if (/hönö/i.test(title)) return 13;
    return 215;
  }
  function fixBoatStatus(row) {
    ...
    if (!when) return;
    var now = Date.now();
    var dur = boatDurMin(title) * 60000;
    var start = isDep ? when.getTime() : when.getTime() - dur;
    var end = isDep ? when.getTime() + dur : when.getTime();
    var away = /kiel/i.test(title) ? "Kiel" : "Frederikshavn";
    if (now < start) ev.textContent = isDep ? "Göteborg" : away;
    else if (now < end) ev.textContent = "På väg";
    else ev.textContent = isDep ? away : "Göteborg";
  }
  ...
  function restyleOne(row, kind) {
    ...
    if (kind === "bat" && /stena danmark/i.test(ident)) city = "Frederikshavn DK";
    if (kind === "bat" && /stena kiel/i.test(ident)) city = "Kiel DE";
