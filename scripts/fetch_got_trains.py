...
def attach_last_seen(trains):
    ...
    xml = (
        ...
        "<INCLUDE>TimeAtLocation</INCLUDE></QUERY></REQUEST>"
    ) % (KEY, ors)
    payload = tv_post(xml)
    best = {}
    for block in ((payload or {}).get("RESPONSE") or {}).get("RESULT") or []:
        ...
        if isinstance(rows, dict):
            rows = [rows]
        for row in rows:
            tid = row.get("AdvertisedTrainIdent")
            ts = row.get("TimeAtLocation")
...
def attach_routes(trains):
    ...
    xml = (
        ...
        "orderby=\"AdvertisedTimeAtLocation\" limit=\"1500\">"
        "<FILTER><AND>"
        ...
        "<OR>%s</OR></AND></FILTER>"
        "<INCLUDE>AdvertisedTrainIdent</INCLUDE><INCLUDE>LocationSignature</INCLUDE>"
        "<INCLUDE>AdvertisedTimeAtLocation</INCLUDE><INCLUDE>EstimatedTimeAtLocation</INCLUDE>"
        "<INCLUDE>TimeAtLocation</INCLUDE><INCLUDE>TrackAtLocation</INCLUDE>"
        "<INCLUDE>Canceled</INCLUDE></QUERY></REQUEST>"
    ) % (KEY, ors)
    payload = tv_post(xml)
    by = {}
    for block in ((payload or {}).get("RESPONSE") or {}).get("RESULT") or []:
        ...
        for row in rows:
            tid = row.get("AdvertisedTrainIdent")
            sig = row.get("LocationSignature") or ""
            ...
            if not prev or str(t) < str(prev.get("t") or "9999"):
                by[tid][sig] = {
                    "sig": sig,
                    "t": t,
                    "act": row.get("TimeAtLocation") or "",
                    "est": row.get("EstimatedTimeAtLocation") or "",
                    "sched": row.get("AdvertisedTimeAtLocation") or "",
                    "track": row.get("TrackAtLocation") or "",
                }
    for f in trains:
        ...
        f["stops"] = [{
            "sig": s["sig"],
            "name": station_name(s["sig"]) or s["sig"],
            "t": s.get("t") or "",
            "act": s.get("act") or "",
            "est": s.get("est") or "",
...
def tv_query():
    xml = (
        "<REQUEST><LOGIN authenticationkey=\"%s\" />"
        "<QUERY objecttype=\"TrainAnnouncement\" schemaversion=\"1.9\" "
        "orderby=\"AdvertisedTimeAtLocation\" limit=\"400\">"
        "<FILTER><AND>"
...
def loc_sig(item):
    ...
    if isinstance(item, dict):
        return item.get("LocationName") or item.get("LocationSignature") or ""
    return ""
...
def loc_list(raw):
    ...
    if isinstance(raw, list):
        return [loc_sig(x) for x in raw if loc_sig(x)]
...
def product_name(f):
    ...
    for p in info:
        if isinstance(p, dict):
            bits.append(p.get("Description") or p.get("Code") or "")
        elif p:
            bits.append(str(p))
...
def slim_train(f):
    ...
    prod = product_name(f)
    blob = (str(traffic) + " " + prod).lower()
    snabb = any(w in blob for w in ("snabbtåg", "x 2000", "x2000", "sx2000"))
    return {
        "id": f.get("AdvertisedTrainIdent") or "",
        "dir": direction,
        "other": station_name(other_sig) or other_sig,
        "otherSig": other_sig,
        "from": [station_name(s) or s for s in frm],
        "to": [station_name(s) or s for s in to],
        "sched": f.get("AdvertisedTimeAtLocation") or "",
        "est": f.get("EstimatedTimeAtLocation") or "",
        "act": f.get("TimeAtLocation") or "",
        "track": f.get("TrackAtLocation") or "",
        "canceled": bool(f.get("Canceled")),
        "product": prod,
        "traffic": traffic,
        "snabb": snabb,
    }
...
def announcements(payload):
    out = []
    results = ((payload or {}).get("RESPONSE") or {}).get("RESULT") or []
...
def merge_keep(old, new):
    ...
    for f in list(old or []) + list(new or []):
        ...
        prev = by.get(key)
        if prev and f.get("stops") and not prev.get("stops"):
            by[key] = f
        elif prev and prev.get("stops") and not f.get("stops"):
            merged = dict(prev)
            merged.update({k: v for k, v in f.items() if k != "stops"})
            by[key] = merged
        else:
            by[key] = f
